...
if os.path.exists(env_path):
    ...
    print(f".env file not found at: {env_path}. Ensure it exists or set environment variables manually.")
...
class Settings:
    ...
    if TELEGRAM_BOT_TOKEN == "YOUR_DEFAULT_TELEGRAM_TOKEN":
        print("Warning: TELEGRAM_BOT_TOKEN is not set in .env, using default placeholder.")
    ...
    if DATABASE_URL == "postgresql://user:pass@host:port/db":
        print("Warning: DATABASE_URL is not set in .env, using default placeholder.")

    OPENAI_API_KEY: str = os.getenv("OPENAI_API_KEY", "YOUR_DEFAULT_OPENAI_KEY")
    if OPENAI_API_KEY == "YOUR_DEFAULT_OPENAI_KEY":
        print("Warning: OPENAI_API_KEY is not set in .env, using default placeholder.")
    ...
    if ADMIN_IDS_STR:
        try:
            ...
            print(f"Warning: ADMIN_IDS in .env ('{ADMIN_IDS_STR}') contains non-integer values. Please provide comma-separated integers.")
            ADMIN_IDS = []
